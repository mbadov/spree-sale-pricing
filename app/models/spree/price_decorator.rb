Spree::Price.class_eval do
  ...
  def put_on_sale(value, start_at = Time.now, end_at = nil, enabled = true)
    new_sale(value, start_at, end_at, enabled).save
  end
  alias :create_sale :put_on_sale
  
  def new_sale(value, start_at = Time.now, end_at = nil, enabled = true)
    Spree::SalePrice.new({ price: self, value: value, start_at: start_at, end_at: end_at, enabled: enabled })
  end
